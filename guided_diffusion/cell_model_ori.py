...
class Cell_Unet(nn.Module):  
    def __init__(self, input_dim=2, hidden_dim=[2000,1000,500,500], \
        ...
        self.label_embedding = LabelEmbedding(num_classes, hidden_dim[0])
    ...
    def forward(self, x, t, y, context_mask=None, inference=False):  
        if context_mask == None:
                context_mask = torch.bernoulli(torch.zeros_like(y)+0.1).to(t.device)
        # convert context to one hot embedding
        y = nn.functional.one_hot(y, num_classes=self.num_classes).type(torch.float).to(t.device) #[256, 10]
